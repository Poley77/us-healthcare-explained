  ...
  chapters.forEach(chapter => {
    (chapter.diagramIds || []).forEach(id => {
      if (!seen.has(id)) {
        ...
        diagrams.push({
          id, chapter,
          label: id.replace(/^diagram\d+_/, '').replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
        })
      }
    })
  })
    ...
    <div className="max-w-6xl mx-auto px-4 lg:px-8 py-12">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {diagrams.map(({ id, chapter, label }) => (
          <div key={id} onClick={() => setLightboxSrc(`/diagrams/${id}.png`)}
            ...
            <div className="aspect-video bg-gray-50 flex items-center justify-center overflow-hidden relative">
              <img src={`/diagrams/${id}.png`} alt={label}
            ...
            <div className="px-4 py-3">
              <div className="text-sm font-medium text-gray-800 font-sans mb-0.5">{label}</div>
              <div className="text-xs text-gray-400 flex items-center gap-1.5 font-sans">
                <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: chapter.layerColor }} />
                {chapter.title}
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 lg:p-8"
          ...
          <div className="relative max-w-5xl w-full" onClick={e => e.stopPropagation()}>
            <button onClick={() => setLightboxSrc(null)}
              className="absolute -top-10 right-0 text-white/70 hover:text-white font-sans text-sm">✕ Close</button>
            <img src={lightboxSrc} alt="" className="w-full h-auto rounded-xl shadow-2xl" />
          </div>
        </div>
      )}
    </div>
